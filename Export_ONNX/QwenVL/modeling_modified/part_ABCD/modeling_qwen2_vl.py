...
try:
    from export_config import WIDTH_FACTOR, HEIGHT_FACTOR, IMAGE_RESIZE
except:
    # Default Values if import failed
    HEIGHT_FACTOR = 10                                      # Adjust this value to determine the resize shape.
    WIDTH_FACTOR = 10                                       # Adjust this value to determine the resize shape.
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, head_dim_half, mrope_section, unsqueeze_dim=1):
...
class VisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        return self.proj(torch.matmul(nn.functional.softmax(torch.matmul(((q * rotary_pos_emb_cos) + (rotate_half(q, self.head_dim_half) * rotary_pos_emb_sin)).transpose(0, 1),
                                                                         ((k * rotary_pos_emb_cos) + (rotate_half(k, self.head_dim_half) * rotary_pos_emb_sin)).permute(1, 2, 0))
                                                            * self.head_dim_factor, dim=-1, dtype=torch.float32), v.transpose(0, 1)).transpose(0, 1).contiguous().view(-1, embed_dim))
...
class Qwen2VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        self.attn = QWEN2_VL_VISION_ATTENTION_CLASSES[attn_implementation](
            self.embed_dim, num_heads=config.num_heads
        )
        self.mlp = VisionMlp(dim=self.embed_dim, hidden_dim=mlp_hidden_dim, hidden_act=config.hidden_act)
    ...
    def forward(self,
        ...
        hidden_states += self.attn(self.norm1(hidden_states), rotary_pos_emb_cos, rotary_pos_emb_sin, pixel_len, self.embed_dim)
...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        query_states, key_states = apply_multimodal_rotary_pos_emb(
            query_states, key_states, cos, sin, self.head_dim_half, self.rope_scaling["mrope_section"]
        )
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.rotary_pos_emb_cos, self.rotary_pos_emb_sin = self.rot_pos_emb(torch.tensor([[1, HEIGHT_FACTOR * 2, WIDTH_FACTOR * 2]], dtype=torch.int32))

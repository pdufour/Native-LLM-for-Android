# https://github.com/huggingface/transformers.js/blob/main/scripts/quantize.py
...
BLOCK_LIST_OPS = ['DynamicQuantizeLinear', 'DequantizeLinear', 'Resize']
...
def quantize_q8(
    ...
    quantizer = ONNXQuantizer(
        ...
        nodes_to_quantize=[],
        nodes_to_exclude=[],
        op_types_to_quantize=[op for op in list(IntegerOpsRegistry.keys())
                            if op not in BLOCK_LIST_OPS],  # Exclude blocked ops
...
def quantize_fp16(
    ...
    model_fp16 = float16.convert_float_to_float16(
        model,
        keep_io_types=True,
        op_block_list=['Conv3D', 'Conv3DNaive', 'Conv', 'LayerNorm', 'LayerNormalization'],
...
def quantize_bnb4(
    ...
    quantizer = MatMulBnb4Quantizer(
        ...
        # nodes_to_exclude=BLOCK_LIST_OPS
    )
    quantizer.process()
    check_and_save_model(quantizer.model.model, save_path)
    return quantizer.model.model
...
def quantize(input_folder, output_folder, quantization_args: QuantizationArguments):
    ...
    for model_path in (progress_models := tqdm(model_names_or_paths)):
        ...
        for mode in (progress := tqdm(quantization_args.modes)):
            ...
            progress.set_description(f" - Loading model")
